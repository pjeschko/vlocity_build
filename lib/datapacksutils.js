...
VLOCITY_BUILD_SALEFORCE_API_VERSION = '44.0';
...
DataPacksUtils.prototype.printJobStatus = function(jobInfo, forceWrite) {
    ...
    if (jobInfo.shouldSendUpdates) {
        this.updateStatusIPC(logInfo);
    }
    ...
    try {
        if (forceWrite || !this.lastWriteTimeLog || (Date.now() - this.lastWriteTimeLog > 60000)) {
            ...
            fileUtil.outputFileSyncSafe('VlocityBuildLog.yaml', yaml.dump(logInfo, { lineWidth: 1000 }));
            //fs.copySync('VlocityBuildLog.yaml', path.join(this.vlocity.tempFolder, 'logs', jobInfo.logName));
            fileUtil.copySyncSafe('VlocityBuildLog.yaml', path.join(this.vlocity.tempFolder, 'logs', jobInfo.logName));
            //fs.outputFileSync('VlocityBuildErrors.log', errorLog);
            fileUtil.outputFileSyncSafe('VlocityBuildErrors.log', errorLog);
...
DataPacksUtils.prototype.saveCurrentJobInfo = async function(jobInfo, force) {
    if (!jobInfo.skipSaveJobInfo && (force || !this.lastWriteTimeJobInfo || (Date.now() - this.lastWriteTimeJobInfo > 60000))) {
        ...
        try {
            ...
            fileUtil.outputFileSyncSafe(CURRENT_INFO_FILE, JSON.stringify(jobInfo, null, 4), 'utf8');
            //await fs.copy(CURRENT_INFO_FILE, CURRENT_INFO_FILE + '.bak');
            fileUtil.copySyncSafe(CURRENT_INFO_FILE, CURRENT_INFO_FILE + '.bak');
...
DataPacksUtils.prototype.retrieveSalesforce = async function(jobInfo) {
    ...
    let allMetadata = await this.vlocity.jsForceConnection.metadata.describe(VLOCITY_BUILD_SALEFORCE_API_VERSION); 
    ...
    if (jobInfo.specificManifestKeys) {
        ...
        for (var metaDesc of allMetadata.metadataObjects) {
            ...
            } else if ((jobInfo.salesforceMetadataTypes && !jobInfo.salesforceMetadataTypes.includes(metaDesc.xmlName)) 
            ...
            deploymentOptions.metadata += metaDesc.xmlName;

            if (metaDesc.xmlName == 'Layout') {
                allPromises.push(this.getFromToolingAPI(metaDesc.xmlName));
            } else {
...
DataPacksUtils.prototype.getFromToolingAPI = async function(objectType) {

    var metadata = '';

    let metaList = await this.vlocity.jsForceConnection.tooling.sobject(objectType)
                            .find({ NamespacePrefix: this.vlocity.namespace })
                            .execute();

    for (var met of metaList) {

        if (metadata) {
            metadata += ',';
        }

        metadata += `${objectType}:${met.FullName}`;
    }
...
DataPacksUtils.prototype.retrieveByType = async function(deploymentOptions, metaDesc, jobInfo) {
    ...
    while (tries < 3) {
        try {
            ...
            let metaList = await this.vlocity.jsForceConnection.metadata.list([{ type: metaDesc.xmlName, folder: null }], VLOCITY_BUILD_SALEFORCE_API_VERSION);
...
DataPacksUtils.prototype.getSFDXProject = function(jobInfo) {
    ...
    if (!fs.existsSync('sfdx-project.json') || jobInfo.deleteSfdxProject || jobInfo.resetSfdxProjectLocation) { 
        ...
        var salesforceProject = {
            ...
            "sourceApiVersion": VLOCITY_BUILD_SALEFORCE_API_VERSION
        };
        ...
        fileUtil.outputFileSyncSafe(path.join('sfdx-project.json'), JSON.stringify(salesforceProject), { encoding: 'utf8' });
        /*
        ...
        */
        fileUtil.outputFileSyncSafe(path.join(sfdxTempFolder, '..', 'sfdx-project.json'), JSON.stringify({
            "packageDirectories": [
                {
                    "path": 'salesforce_sfdx',
                    "default": true
