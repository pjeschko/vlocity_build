...
ValidationTest.prototype.validate = async function(jobInfo, currentContextData) {
    ...
    await this.getInitialDataPackInfo(currentContextData);
...
ValidationTest.prototype.getInitialDataPackInfo = function(currentContextData) {
    this.vlocity.initialTestInfo = {
        vlocityDataPackKeyToRecordSourceKey: {},
        vlocityRecordSourceKeyToDataPackKey: {}
    };

    for (var dataPack of currentContextData) {
        this.vlocity.initialTestInfo.vlocityDataPackKeyToRecordSourceKey[dataPack.VlocityDataPackKey] = [];

        var dataField = this.vlocity.datapacksutils.getDataField(dataPack);

        if (dataField) {
            for (var sobject of dataPack.VlocityDataPackData[dataField]) {
                this.getInitialDataPackInfoFromSObject(dataPack.VlocityDataPackKey, sobject);
            }
        }
    }
}

ValidationTest.prototype.getInitialDataPackInfoFromSObject = function(dataPackKey, sobject) {
   
    if (sobject.VlocityDataPackType == 'SObject') {

        if (!this.vlocity.initialTestInfo.vlocityRecordSourceKeyToDataPackKey[sobject.VlocityRecordSourceKey]) {
            this.vlocity.initialTestInfo.vlocityRecordSourceKeyToDataPackKey[sobject.VlocityRecordSourceKey]= [];
        }

        this.vlocity.initialTestInfo.vlocityRecordSourceKeyToDataPackKey[sobject.VlocityRecordSourceKey].push(dataPackKey);

        for (var field in sobject) {
            if (Array.isArray(sobject[field])) {
                for (var child of sobject[field]) {
                    this.getInitialDataPackInfoFromSObject(dataPackKey, child);
                }
            }
        }
    }
...
ValidationTest.prototype.saveReportLocally = function() {
    ...
    if (!this.vlocity.utilityservice.isEmptyObject(testReport)) {
        ...
        fileUtil.outputFileSyncSafe(path.join(this.jobInfo.tempFolder, 'validationTest', this.jobInfo.logName), yaml.dump(testReport), 'utf8');
    }
...
ValidationTest.prototype.checkAllReferenceFields = function(action) {

    this.vlocity.initialTestInfo.vlocityDataPackKeyToRecordSourceKey[action.vlocityDataPackKey] = [];

    this.checkAllReferenceFieldsSObject(action.vlocityDataPackKey, action.dataPack);
}

ValidationTest.prototype.checkAllReferenceFieldsSObject = function(dataPackKey, sobject) {
    ...
    if (sobject.VlocityDataPackType == 'SObject') {
        ...
        for (var field in sobject) {
            if (Array.isArray(sobject[field])) {
                for (var child of sobject[field]) {
                    this.checkAllReferenceFieldsSObject(dataPackKey, child);
                }
            } else if (sobject[field] && sobject[field] instanceof Object && sobject[field].VlocityLookupRecordSourceKey) {

                if (!sobject[field].VlocityRecordSObjectType == 'RecordType') {
                    if (!this.vlocity.initialTestInfo.vlocityRecordSourceKeyToDataPackKey[sobject[field].VlocityLookupRecordSourceKey]) {
                        this.failTest('No Match Found for ' + message + ' Actual Result ' + actual + ' Expected Result ' + expected, action);
                    }
                }
            }
        }
