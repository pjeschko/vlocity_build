...
DataPacksJob.prototype.intializeJobInfo = async function(jobInfo, action) {
    ...
    this.vlocity.datapacksutils.loadIgnoreFile(jobInfo.projectPath);  
};
    ...
    try {
        ...
        if (jobInfo.jobAction == 'GetAllAvailableExports') {
            ...
            try {
                ...
                fileUtil.outputFileSyncSafe('VlocityBuildLog.yaml', yaml.dump({ manifest: manifestArray }, { lineWidth: 1000 }));
    ...
    if (jobInfo.queries) {
        ...
        try {
            ...
            for (var queryData of jobInfo.queries) {
                ...
                if (!queryData || (!jobInfo.specificManifestKeys && queryData.manifestOnly)) {
                    continue;
                }
    ...
    if (jobInfo.buildFile) {
        ...
        if (dataJson) {
            ...
            if (fileName) {
                ...
                fileUtil.outputFileSyncSafe(fileName, stringify(dataJson, { space: 4 }), 'utf8');
            }
            ...
            if (fileName.indexOf('.resource') > 0) {
                ...
                //    'utf8');
                fileUtil.outputFileSyncSafe(fileName + '-meta.xml', '<?xml version="1.0" encoding="UTF-8"?><StaticResource xmlns="http://soap.sforce.com/2006/04/metadata"><cacheControl>Public</cacheControl><contentType>text/json</contentType></StaticResource>',
                    'utf8');
            }
...
DataPacksJob.prototype.expandFile = async function(jobInfo) {
    await this.vlocity.datapacksexpand.expandFile(jobInfo.projectPath + '/' + jobInfo.expansionPath, jobInfo.buildFile, jobInfo);
};

DataPacksJob.prototype.deployDataPackMetadataConfigurations = async function(jobInfo) {
    var dataPackMetadataLatest = await this.vlocity.datapacksutils.loadFilesFromDir(path.join(__dirname, '..', 'DataPackMetadata/latest'));
    ...
    for (var customMetadataType in dataPackMetadataLatest) {
        ...
        for (var recordName in metadataRecordsLatest) {
            ...
            if (customMetadataType === 'VlocityDataPackConfiguration') {
                ...
                if (!dataPackConfigurations.hasOwnProperty(recordName)) {
...
DataPacksJob.prototype.updateSettings = async function(jobInfo) {
    ...
    if (jobInfo.force || result.records.length == 0) {
        await this.deployDataPackMetadataConfigurations(jobInfo);
        ...
        if (result.VlocityDataPackId) {
            ...
            for (var dataPack of dataPackData.dataPacks) {
                ...
                if (self.vlocity.datapacksbuilder.savedBulkRecords[dataPackKey]) {
                    ...
                    if (dataPack.VlocityDataPackType === 'OmniScript') {
            ...
            if (deltaCheckJobInfo.deltaCheckResults) {
                for (var dataPackKey in deltaCheckJobInfo.deltaCheckResults) {
                    if (deltaCheckJobInfo.deltaCheckResults[dataPackKey].status == 'Error') {
                        ...
                        if (jobInfo.specificManifestKeys 
                            && this.vlocity.datapacksbuilder.parentData[dataPackKey] 
                            && jobInfo.autoDeployDependencies) {
                            for (var parentKey of this.vlocity.datapacksbuilder.parentData[dataPackKey]) {
                                if (jobInfo.specificManifestKeys.indexOf(parentKey) == -1) {
                                    VlocityUtils.verbose('Adding Dependency', parentKey);
                                    jobInfo.specificManifestKeys.push(parentKey);
                                    checkAndAddParents = true;
                                    jobInfo.currentStatus[dataPackKey] = 'Ready';
                                }
                            }
                        } else if (jobInfo.currentStatus[dataPackKey] == 'Error' && deltaCheckJobInfo.deltaCheckResults[dataPackKey].errorMessage) {
                            jobInfo.errors.push(deltaCheckJobInfo.deltaCheckResults[dataPackKey].errorMessage);
                            jobInfo.hasError = true;
                        }
                    }
...
DataPacksJob.prototype.checkDiffs = async function(jobInfo, currentLocalFileData, targetOrgRecordsHash) {
   ...
   // await Promise.all([
   //    fs.outputFile(path.join(this.vlocity.tempFolder, 'diffs/localFolderFiles.json'), stringify(currentFiles, { space: 4 })),
   //    fs.outputFile(path.join(this.vlocity.tempFolder, 'diffs/targetOrgFiles.json'), stringify(exportedFiles, { space: 4 }))
   // ]);
   fileUtil.outputFileSyncSafe(path.join(this.vlocity.tempFolder, 'diffs/localFolderFiles.json'), stringify(currentFiles, { space: 4 }));
   fileUtil.outputFileSyncSafe(path.join(this.vlocity.tempFolder, 'diffs/targetOrgFiles.json'), stringify(exportedFiles, { space: 4 }));
};
...
DataPacksJob.prototype.getDiffsAndDeploy = async function(jobInfo) {
    ...
    if (currentFileData && currentFileData.dataPacks) {
        ...
        for (var dataPack in currentFileData.dataPacks) {
            allTargetDataPacks[dataPack.VlocityDataPackKey] = dataPack;
...
DataPacksJob.prototype.reExpandAllFilesAndRunJavaScript = async function(jobInfo) {
    VlocityUtils.verbose('Running All JavaScript', jobInfo.javascript);
