...
DataPacksExpand.prototype.generateFolderOrFilename = function(filename, extension) {
    if (!filename) return 'MissingName-' + this.vlocity.datapacksutils.guid();
...
DataPacksExpand.prototype.processDataPack = async function(dataPackData, jobInfo, isPagination) {
    ...
    if (dataPackData.VlocityDataPackData) {
        ...
        if (dataField && !Array.isArray(dataPackData.VlocityDataPackData[dataField])) {
            VlocityUtils.error('processDataPack', dataPackData.VlocityDataPackKey , dataPackData.VlocityDataPackName);        
        ...
        if (dataField && dataPackData.VlocityDataPackData[dataField]) {
            ...
            for (var sobjectData of dataPackData.VlocityDataPackData[dataField]) {
                ...
                if (!isPagination) {
                   ...
                   fileUtil.emptyDirSyncSafe(self.generateFolderPath(dataPackType, parentName));
                }
                    ...
                    try {
                        if (expansionData) {
                            if (expansionType == "list") {
                                ...
                                self.getReferencedDataPacks(extension, dataPackType, expansionData, jobInfo);
...
DataPacksExpand.prototype.writeFile = async function(dataPackType, parentName, filename, fileType, fileData, isPagination, encoding) {
    ...
    fileUtil.outputFileSyncSafe(fullFilePath, fileData, { "encoding": encoding });
...
DataPacksExpand.prototype.expandSFDX = async function(dataPack, jobInfo) {
    if (dataPack.SFDXData) {
        ...
        var fullFilePath = path.join(jobInfo.sfdxFolderPath, 'main', 'default', dataPack.VlocityDataPackKey).replace(/%vlocity_namespace%/g, this.vlocity.namespace);

        if (this.vlocity.namespace) {
            dataPack.SFDXData = dataPack.SFDXData.replace(/%vlocity_namespace%/g, this.vlocity.namespace);
        }

        VlocityUtils.success('Wrting SFDX File', fullFilePath);

        if (dataPack.SFDXDataType == 'staticresources') {
            var allFiles = JSON.parse(dataPack.SFDXData);

            for (resourceFile of allFiles) {
                //await fs.outputFile(path.join(jobInfo.sfdxFolderPath, 'main', 'default', resourceFile.filePath), resourceFile.fileData, { "encoding": resourceFile.encoding });
                fileUtil.outputFileSyncSafe(path.join(jobInfo.sfdxFolderPath, 'main', 'default', resourceFile.filePath), resourceFile.fileData, { "encoding": resourceFile.encoding });
            }
        } else {
            //await fs.outputFile(fullFilePath, dataPack.SFDXData, { "encoding": dataPack.SFDXDataType });
            fileUtil.outputFileSyncSafe(fullFilePath, dataPack.SFDXData, { "encoding": dataPack.SFDXDataType });
        }
