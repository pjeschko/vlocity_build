    ...
    if (comparisonInfo.hasComparison) {
        if (options.target == 'local') {
            ...
            if (!fs.existsSync(path.join(options.projectPath, 'sfdx-project.json'))) {
                ...
                */
               fileUtil.outputFileSyncSafe(path.join(options.projectPath, 'sfdx-project.json'), JSON.stringify({
                    "packageDirectories": [
                        {
                            "path": 'salesforce_sfdx',
                            "default": true
...
Vlocity.revertChange = async function(options) {
    ...
    try {
        ...
        fileUtil.outputFileSyncSafe(comparisonInfo.sourceFileCurrent, JSON.stringify(comparisonFileSource), 'utf8');
...
Vlocity.compare = async function(options) {
    ...
    if (!fs.existsSync(comparisonInfo.sourceFileCurrent)) {
        ...
        if (options.sourceData) {
            ...
            fileUtil.outputFileSyncSafe(comparisonInfo.sourceFileCurrent, JSON.stringify(options.sourceData, null, 4), 'utf8');
        }
    ...
    if (!fs.existsSync(comparisonInfo.targetFileCurrent)) {
        ...
        if (options.targetData) {
            ...
            fileUtil.outputFileSyncSafe(comparisonInfo.targetFileCurrent, JSON.stringify(options.targetData, null, 4), 'utf8');
        }
    ...
    fileUtil.outputFileSyncSafe(comparisonInfo.diffsFileCurrent, JSON.stringify(differences, null, 4), 'utf8');
